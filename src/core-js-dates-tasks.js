...
function getTime(date) {
  ...
  const returnHms = (paramsArr) => {
    let hms = '';
    for (let i = 0; i < paramsArr.length; i += 1) {
      hms = hms.concat(decimalNumber(paramsArr[i])).concat(':');
    }
    return hms.substring(0, hms.length - 1);
  };

  return returnHms(datePieces);
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= lastDay; i += 1) {
    const itarateDate = new Date(year, month - 1, i).getDay();
    if (itarateDate === 0 || itarateDate === 6) {
      counter += 1;
    }
  }
  return counter;
...
function getWeekNumberByDate(date) {
  ...
  let actualStart = new Date(year, 0, 1);
  let temp = actualStart;
  let offset = 0;
  while (temp.getDay() !== 0) {
    offset += 1;
  ...
  return offset > 0
    ...
    : Math.ceil((input - actualStart) / week);
}
